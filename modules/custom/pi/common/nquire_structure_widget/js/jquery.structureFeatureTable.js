...
(function($) {
	...
	var methods = {
		...
		_buildColumn: function(column) {
			...
			this.find('tr').not('.structure-feature-list').each(function() {
				var row = $(this);
				var notEditable = row.attr('not_editable');
				...
				if (options.forItemType === 'all' || options.forItemType === row.attr('type')) {
					...
					if (notEditable) {
						if (value) {
							valueCell.append(options.values[value]);
						}
					} else {
			...
			cell.find('a').nQuireTooltip({
				creationCallback: function(content) {
					for (var key in values) {
						...
						if (key !== currentValue) {
							element = self.structureFeatureTable('_createLink', values[key], false)
											.attr('value', key)
											.click(function() {
